...
func (sm *StreamsManager) generateStreamId(streamName string, id string) (string, int64, int, error) {
	...
	lastEntry := stream.recordList.Back()
	if lastEntry == nil {
		return sm.generateFirstId(msTime, seqNum, wildcardNum)
	}
 ...
 * @param seqNum int - the sequenceNumber part of the new stream ID
 ...
 * @return error - the error if there is one
 */
func (sm *StreamsManager) generateFirstId(msTime int64, seqNum int, wildcardNum int) (string, int64, int, error) {
	if wildcardNum == AutoGeneratedSeq && msTime > 0 {
		// If the sequence number is auto-generated and milliseconds time is provided,
		// generate an ID with sequence number 0.
		return fmt.Sprintf("%d-0", msTime), msTime, 0, nil
	} else if msTime == 0 && wildcardNum == AutoGeneratedSeq {
		// If both milliseconds time is 0,
		// default to 0 milliseconds time and sequence number 1.
		return fmt.Sprintf("%d-1", msTime), msTime, 1, nil
	} else if wildcardNum == AutoGeneratedTimeAndSeq {
		// If wildcardNum indicates automatic time and sequence number generation,
		// use the current system time and start with sequence number 0.
...
/*
 	* createStreamEntryMessages converts stream records to RESP messages
	* @param records []store.StreamRecord - the stream records to convert
	* @return []RESP.RESPMessage - the RESP messages
*/
func (sm *StreamsManager) CreateStreamEntryMessages(records []StreamRecord) []RESP.RESPMessage {
	...
	for i, record := range records {
		...
		// Create entry with ID and key-value pairs
